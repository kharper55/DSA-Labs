...
#include "HeapPriorityQueue.hpp"    // For heap data structure
...
// Prototypes
void buildMaxHeapFromArr(const uint32_t * arr, const uint32_t arrSize, HeapPriorityQueue<uint32_t, std::greater<uint32_t>>& maxHeap);
void heapSort(uint32_t * arr, const uint32_t arrSize, HeapPriorityQueue<uint32_t, std::greater<uint32_t>>& maxHeap);
                        ...
                        if (!invalidEntry) {
                            ...
                            for (int i = 1; i <= static_cast<int>(numIterations - 48); i++) {
                                ...
                                switch(algo) {
                                    ...
                                    case(HEAP):
                                        {
                                        HeapPriorityQueue<uint32_t, std::greater<uint32_t>> maxHeap;
                                        start = chrono::high_resolution_clock::now();
                                        heapSort(fileReadBuff, FILE_SIZES[sortFileSize], maxHeap);
                                        stop = chrono::high_resolution_clock::now();
                                        break;
                                        }
